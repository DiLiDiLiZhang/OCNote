...
// Object-C的一些基础
1.OC是一门动态运行时语言
    静态、动态是相对的，这里动态语言指的是不需要在编译时确定所有的东西，它会将一些工作放在代码运行时才处理而并非编译时，有很多类和成员变量在我们编译的时是不知道的，而在运行时，我们所编写的代码会转换成完整的确定的代码运行，而且在运行时还可以动态的添加变量、方法和类。因此，编译器是不够的，我们还需要一个运行时系统(Runtime system)来处理编译后的代码。
